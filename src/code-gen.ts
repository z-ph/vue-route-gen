...
export function renderRoutesFile({
   ...
   */
  function collectMetaKeys(route: RouteEntry | RouteData['routes'][0]) {
  ...
  const allRoutesByName = new Map<string, RouteEntry | RouteData['routes'][0]>();

  /**
   * Recursively build map of all routes by name
   */
  function buildRouteMap(route: RouteEntry | RouteData['routes'][0]) {
